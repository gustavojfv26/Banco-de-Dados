...
--
-- Host: 127.0.0.1
-- Tempo de geração: 20/02/2025 às 21:04
...
DROP PROCEDURE IF EXISTS `Inserirloja`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `Inserirloja` (IN `cod_loja` INT, IN `contato_loja` VARCHAR(15), IN `email_loja` VARCHAR(45), IN `cep` VARCHAR(8), IN `dono_loja` VARCHAR(45), IN `registro` VARCHAR(45), IN `cnpj` VARCHAR(14), IN `funcionario_id_funcionario` INT, IN `fornecedor_id_fornecedor` INT, IN `fornecedor_cliente_cpf_cliente` INT, IN `fornecedor_cliente_produto_cod_produto` INT)   BEGIN
INSERT INTO funcionario (cod_loja, contato_loja, email_loja, cep, dono_loja, registro, cnpj, funcionario_id_funcionario, fornecedor_id_fornecedor, fornecedor_cliente_cpf_cliente, fornecedor_cliente_produto_cod_produto)
VALUES (
	cod_loja,
    contato_loja, 
    email_loja,
    ...
    fornecedor_cliente_cpf_cliente,
    fornecedor_cliente_produto_cod_produto
);
END$$
